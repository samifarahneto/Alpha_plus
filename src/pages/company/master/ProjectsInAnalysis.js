...
const ProjectsInAnalysis = () => {
  ...
  const [rowsPerPage, setRowsPerPage] = useState(() => {
    const savedRowsPerPage = localStorage.getItem(
      "projectsInAnalysisRowsPerPage"
    );
    return savedRowsPerPage ? parseInt(savedRowsPerPage) : 10;
  });
  const [sortConfig] = useState(() => {
    const savedSortConfig = localStorage.getItem(
      "projectsInAnalysisSortConfig"
    );
  ...
  useEffect(() => {
    ...
    const firestore = getFirestore();
    const collections = ["b2bapproved", "b2bprojectspaid", "b2cprojectspaid"];
  ...
  const handleRowClick = (row) => {
    ...
    const cleanProjectData = {
      ...
      project_status: row.project_status || "Em Análise",
...
export default ProjectsInAnalysis;
