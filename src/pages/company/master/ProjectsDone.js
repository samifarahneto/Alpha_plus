...
const ProjectsDone = () => {
  ...
  const [rowsPerPage, setRowsPerPage] = useState(() => {
    const savedRowsPerPage = localStorage.getItem("projectsDoneRowsPerPage");
    return savedRowsPerPage ? parseInt(savedRowsPerPage) : 10;
  });
  const [sortConfig] = useState(() => {
    const savedSortConfig = localStorage.getItem("projectsDoneSortConfig");
  ...
  const columns = [
    ...
    { id: "clientOrigin", label: "Cliente Origem" },
    { id: "type", label: "Tipo" },
    ...
    try {
      const unsubscribeFunctions = collections.map((collectionName) => {
        ...
        return onSnapshot(
          collectionRef,
          (snapshot) => {
            // Filtrar apenas projetos com translation_status ou project_status "Finalizado"
            const filteredProjects = snapshot.docs
              .filter(
                (doc) =>
                  doc.data().translation_status === "Finalizado" ||
                  doc.data().project_status === "Finalizado"
              )
              .map((doc) => ({
                id: doc.id,
                ...doc.data(),
  ...
  const handleRowClick = (row) => {
    ...
    const cleanProjectData = {
      ...
      project_status: row.project_status || "Finalizado",
  ...
  const formattedData = React.useMemo(() => {
    return paginatedData.map((row) => ({
      ...
      projectName: row.projectName || "Sem Nome",
      ...
      paymentStatus: renderPaymentStatusBadge(row.payment_status || "N/A"),
      deadline: row.deadlineDate
        ? new Date(row.deadlineDate).toLocaleDateString("pt-BR")
        : "Sem Prazo",
      status: renderStatusBadge(row.project_status || "N/A"),
    }));
        ...
        <div className="w-full">
          ...
          <Pagination
            ...
            onRowsPerPageChange={(value) => {
              ...
              localStorage.setItem("projectsDoneRowsPerPage", value);
            }}
