...
const MasterRefund = () => {
  ...
  const [rowsPerPage, setRowsPerPage] = useState(() => {
    const savedRowsPerPage = localStorage.getItem("masterRefundRowsPerPage");
    return savedRowsPerPage ? parseInt(savedRowsPerPage) : 10;
  });
  const [sortConfig] = useState(() => {
    const savedSortConfig = localStorage.getItem("masterRefundSortConfig");
  ...
  const columns = [
    ...
    { id: "paymentStatus", label: "Pgto", fixed: true },
    { id: "projectStatus", label: "Status" },
    { id: "translationStatus", label: "Tradução" },
  ];
  ...
  useEffect(() => {
    ...
    const firestore = getFirestore();
    const collections = ["b2bprojectspaid", "b2cprojectspaid"];
        ...
        return onSnapshot(
          ...
          (snapshot) => {
            setProjects((prevProjects) => {
              // Filtrar apenas projetos com status de pagamento "Em Reembolso" ou "Reembolsado"
              const filteredProjects = snapshot.docs
                .filter((doc) => {
                  const paymentStatus =
                    typeof doc.data().payment_status === "object"
                      ? doc.data().payment_status.status
                      : doc.data().payment_status;
                  return (
                    paymentStatus === "Em Reembolso" ||
        ...
        <div className="w-full">
          ...
          <Pagination
            ...
            onRowsPerPageChange={(value) => {
              ...
              localStorage.setItem("masterRefundRowsPerPage", value);
            }}
