...
const ClientRefund = () => {
  ...
  const [rowsPerPage, setRowsPerPage] = useState(() => {
    const savedRowsPerPage = localStorage.getItem("clientRefundRowsPerPage");
  ...
  const calculateTotalPages = useCallback((files, row) => {
    // Se há payment_status com páginas de divergência, usar esse valor
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        // Configurar listeners para b2bprojectspaid e b2cprojectspaid
        ...
        collections.forEach((collectionName) => {
          ...
          const unsubscribe = onSnapshot(q, (snapshot) => {
            const newProjects = snapshot.docs
              ...
              .filter((project) => {
                // Filtrar apenas projetos em reembolso ou reembolsados
                const paymentStatus =
  ...
  const handleRowsPerPageChange = (value) => {
    ...
    setCurrentPage(1);
    localStorage.setItem("clientRefundRowsPerPage", value.toString());
  };
